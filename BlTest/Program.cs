 ...
 internal class Program
 {
  ...
  private static void productMethod()
  {
   ...
   switch (choiceProduct)
   {
    ...
    case 2://view product deyails for manager
    ...
    case 6://view product deyails for customer
     Console.WriteLine("enter product ID and your details");
     int.TryParse(Console.ReadLine(), out parse);
     product.ID = parse;
     ...
     while (amount > 0)
     {
      ...
      {
       //ID=
       Name = bl.Product.GetProductDetailsManager(product.ID).Name,
       ProductID = product.ID,
       Price = bl.Product.GetProductDetailsManager(product.ID).Price,//manager
       Amount = amount
      };
   ...
   switch (choiceOrder)
   {
    case 1://add product to the cart
     ...
     while (productID != 0)
     {
      ...
      {
       //ID=
       Name = bl.Product.GetProductDetailsManager(product.ID).Name,//manager
       ProductID = productId,
       Price = bl.Product.GetProductDetailsManager(product.ID).Price,//manager
       Amount = amount
      };
